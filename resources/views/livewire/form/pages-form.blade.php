    ...
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        <div class="mb-4">
            <flux:breadcrumbs>
                ...
                <flux:breadcrumbs.item href="{{ route('pages-list') }}">Daftar Halaman</flux:breadcrumbs.item>
                <flux:breadcrumbs.item href="#">{{ $pageTitle }}</flux:breadcrumbs.item>
            </flux:breadcrumbs>
        </div>
            ...
            <div class="bg-gray-100">
                <div class="max-w-7xl mx-auto">
                    @if (session('ui.message'))
                        <div class="{{ session('ui.message_type') === 'error' ? 'bg-red-500' : 'bg-green-500' }} text-white p-4 rounded-lg mb-4">
                            {{ session('ui.message') }}
                        </div>
                    @endif
                    ...
                    <div class="max-w-12xl mx-auto p-6 bg-white shadow-lg rounded-lg">
                        ...
                        <form wire:submit.prevent="submitForm">
                            <x-select 
                                ...
                                :options="$kategoris->pluck('judul_halaman', 'id')" 
                                defaultOption="Pilih Kategori Halaman" 
                                required />

                            <x-input 
                                id="judul_halaman" 
                                label="Judul Halaman" 
                                wireModel="judul_halaman" 
                                placeholder="Masukkan Judul Halaman"
                                required />

                            <div class="mb-6">
                                <label for="deskripsi_halaman" class="block text-sm font-medium text-gray-700">Deskripsi Halaman</label>
                                <textarea 
                                    wire:model="deskripsi_halaman" 
                                    id="deskripsi_halaman" 
                                    rows="5"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm"
                                    placeholder="Masukkan Deskripsi Halaman"
                                    required></textarea>
                                @error('deskripsi_halaman')
                            ...
                            <x-input 
                                id="ikon_halaman" 
                                label="Ikon Halaman (Font Awesome Class)" 
                                wireModel="ikon_halaman" 
                                placeholder="Contoh: fas fa-home" />

                            <div class="mb-6">
                                <label for="gambar" class="block text-sm font-medium text-gray-700">Gambar Halaman</label>
                                <input 
                                    type="file" 
                                    wire:model="gambar" 
                                    id="gambar" 
                            ...
                            <div class="mb-6">
                                <label class="inline-flex items-center">
                                    <input 
                                        type="checkbox" 
                                        wire:model="status_halaman" 
                                        class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                                    <span class="ml-2 text-sm text-gray-700">Aktifkan Halaman</span>
                                </label>
                            </div>
                            ...
                            <div class="mt-8">
                                <button type="submit" class="w-full px-6 py-3 bg-indigo-600 text-white font-semibold rounded-lg shadow-md hover:bg-indigo-700 focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-50 flex items-center justify-center space-x-2 text-sm">
                                    ...
                                    <span>{{ $pages_id ? 'Update Halaman' : 'Simpan Halaman' }}</span>
                                </button>
                            </div>
                        </form>
                    </div>
